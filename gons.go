...
func externalDNSCheck(wantName string) [4]byte {
	r := &net.Resolver{
		PreferGo: true,
		Dial: func(ctx context.Context, network, address string) (net.Conn, error) {
			...
			return d.DialContext(ctx, network, fmt.Sprintf("%s:53", currentConfig.Server.ExternalDNS))
		},
	}
	ip, err := r.LookupHost(context.Background(), wantName)
	if err != nil {
		return unresolvedAddr
	}
	return ipToBytes(ip[0])
}
...
func ServeDNS(addr *net.UDPAddr, conn *net.UDPConn, msg dnsmessage.Message, rawMsg []byte) {
	...
	if !isPrivateHost(queryNameStr) {
		extResp := externalDNSCheck(queryNameStr)
